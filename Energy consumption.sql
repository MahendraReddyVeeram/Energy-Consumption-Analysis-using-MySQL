...
select country from gdp_3 where year = (select max(year) from gdp_3) order by year desc limit 5;
...
select country, year from gdp_3 order by country,year;
...
FROM consumption
WHERE country IN ('United States', 'China', 'India', 'Germany', 'Japan') -- adjust as needed
...
SELECT country, AVG(per_capita_emission) AS avg_yearly_per_capita_emission
FROM emission_3
GROUP BY country;
